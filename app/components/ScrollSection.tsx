...
export function ScrollSectionProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      let sectionContainingViewportCenter: string | null = null;
      ...
      sectionsRef.current.forEach((element, sectionId) => {
        ...
        if (
          ...
          !sectionContainingViewportCenter
        ) {
          sectionContainingViewportCenter = sectionId;
        }

        // Fall back to the closest visible section near document edges.
        if (
      ...
      const nextActiveSection = sectionContainingViewportCenter ?? closestSection;
